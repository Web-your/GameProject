...
class MyGame(arcade.Window):
    ...
    def setup(self):  # Список списков координат квадратов для рисования
        self.dialog = Dialog((self.width // 4, self.height // 4), file_text=r"example.txt", width=300)
        self.dialog.base_settings["time_appear"] = 0.2
        self.dialog.base_settings["color"] = (255,255,255)
        self.dialog.base_settings["font_size"] = 22
        self.dialog.refresh_data()
        self.keys = set()
        image = arcade.Sprite(EasySprite.load_texture("hero_reaction.png", 3))
        image.center_x = 100
        image.center_y = self.height // 6
        self.sprits = arcade.SpriteList()
        self.sprits.append(image)

    def on_draw(self):
        """Этот метод отвечает за отрисовку содержимого окна"""
        self.clear()
        arcade.draw_lbwh_rectangle_filled(0, 0, self.width, self.height // 3, (0, 0, 0))
        arcade.draw_lbwh_rectangle_outline(0, 0, self.width, self.height // 3, (255, 255, 255), 4)
        self.dialog.draw()
        self.sprits.draw()
    ...
    def on_update(self, delta_time):
        ...
        if arcade.key.ENTER in self.keys:
